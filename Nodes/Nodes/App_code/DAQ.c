...
void sendOilPressure()
{
	...
	while (CAN_Transmit(CAN1, &OilPressureMsg) != CAN_TxStatus_NoMailBox)
		;
...
void sendSteeringAngle()
{
	...
	CAN_RTR_Data, CAN_ID_STD, 3, &data[0]);
	while (CAN_Transmit(CAN1, &SteeringMsg) != CAN_TxStatus_NoMailBox)
		;
...
void sendAccData()
{
	...
	while (CAN_Transmit(CAN1, &AccMsg) != CAN_TxStatus_NoMailBox)
		;
}
